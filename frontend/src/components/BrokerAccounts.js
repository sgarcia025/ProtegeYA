...
const BrokerAccounts = () => {
  ...
  const fetchPlans = async () => {
    try {
      console.log('Fetching plans from:', `${API}/admin/subscription-plans`);
      const response = await axios.get(`${API}/admin/subscription-plans`);
      console.log('Plans response:', response.data);
      console.log('Plans array length:', response.data.length);
              ...
              {selectedTransaction && (
                <div className="mb-4 p-3 bg-red-50 rounded-lg">
                  <p className="text-sm text-red-700">
                    ...
                    <strong>Fecha:</strong> {formatDate(selectedTransaction.created_at)}
                  </p>
                </div>
              )}

              <form onSubmit={deletePayment} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Contacte al administrador para obtener el código de autorización.
                  </p>
                </div>
